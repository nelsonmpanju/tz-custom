...
def get_columns():
    """Return columns for the weekly report."""
    return [
        {"fieldname": "employee", "label": _("Employee No"), "fieldtype": "Link", "options": "Employee", "width": 300},
        {"fieldname": "employee_name", "label": _("Employee Name"), "fieldtype": "Data", "width": 200},
        {"fieldname": "total_days", "label": _("Total Days"), "fieldtype": "Int", "width": 50},
        {"fieldname": "total_worked_hours", "label": _("Total Worked Hours"), "fieldtype": "Data", "width": 150},
        {"fieldname": "overtime_tsh", "label": _("Overtime TSH"), "fieldtype": "Currency", "width": 130},
        {"fieldname": "extra_overtime", "label": _("Extra Overtime"), "fieldtype": "Currency", "width": 130},
        {"fieldname": "total_overtime", "label": _("Total Overtime"), "fieldtype": "Currency", "width": 130},
    ]
...
def get_conditions(filters):
    ...
    # Shift filter removed from SQL - applied in Python after shift determination
...
def get_attendance_data(conditions, filters):
    """Get attendance data, compute daily, then aggregate over the period."""
    ...
    # Process only CHECKIN dates (each checkin date = one work day)
    # This prevents phantom rows from checkout-only dates
    processed_keys = set()

    for key in checkins_by_emp_date:
        ...
        # CRITICAL: Only include records with BOTH checkin AND checkout
        # Missing checkin or checkout = incomplete record, skip entirely
        if not first_checkin or not last_checkout:
            continue

        # Skip if already processed (prevents duplicate rows)
    ...
    # Now aggregate over the period per employee
    # Only complete attendance records (with both checkin and checkout) are included here
    period_sums = defaultdict(lambda: defaultdict(float))
    employee_names = {}

    for row in attendance_data:
        emp = row["employee"]
        # Count days: Each row = 1 checkin/checkout pair = 1 day worked
        # This provides accurate day counts based on actual attendance pairs
        if row["total_hours"] and row["total_hours"] > 0:
            period_sums[emp]["total_days"] += 1
        period_sums[emp]["total_hours"] += row["total_hours"]
        period_sums[emp]["overtime_tsh"] += row["overtime_tsh"]
        period_sums[emp]["extra_overtime"] += row["extra_overtime"]
        period_sums[emp]["total_overtime"] += row["total_overtime"]

        employee_names[emp] = row["employee_name"]

    aggregated_data = []
    for emp, sums in period_sums.items():
        hours_display = format_hours(sums["total_hours"])

        # Apply currency approximation: 1-399→000, 400-799→500, 800-999→1000
        rounded_overtime = approximate_currency(sums["overtime_tsh"])
        rounded_extra = approximate_currency(sums["extra_overtime"])
        rounded_total = rounded_overtime + rounded_extra

        agg_row = {
            "employee": emp,
            "employee_name": employee_names[emp],
            "total_days": sums["total_days"],
            "total_worked_hours": hours_display,
            "overtime_tsh": rounded_overtime,
            "extra_overtime": rounded_extra,
            "total_overtime": rounded_total,
        }
        aggregated_data.append(agg_row)

    # Sort by employee
    aggregated_data.sort(key=lambda x: x["employee"])

    # Convert to list of lists
    return [[row["employee"], row["employee_name"], row["total_days"], row["total_worked_hours"],
             row["overtime_tsh"], row["extra_overtime"], row["total_overtime"]]
...
def approximate_currency(amount):
    """
    Approximate currency to nearest 500/1000.

    Rules:
    - Last 3 digits 1-399   → round down to 000
    - Last 3 digits 400-799 → round to 500
    - Last 3 digits 800-999 → round up to 1000

    Examples:
        3,560 → 3,500   (560 is 400-799 → 500)
        4,350 → 4,000   (350 is 1-399 → 000)
        4,800 → 5,000   (800 is 800-999 → +1000)
        16,770 → 16,500 (770 is 400-799 → 500)
        7,500 → 7,500   (500 is 400-799 → 500)
        9,270 → 9,000   (270 is 1-399 → 000)
    """
    if not amount or amount <= 0:
        return 0

    amount = flt(amount, 0)
    base = int(amount // 1000) * 1000
